...
goal_map = PointStamped() #goal rel map
...
def set_pose(msg): #get roation of the rig
    global pose
    pose.linear.x = msg.linear.x
    pose.linear.y = msg.linear.y
...
def coord_transform(): #use the measured pose and goal in the map to
    ...
    dx = goal_map.point.x - pose.linear.x
    #dy = ym-yr  distance from robot in unrotated coord.sys
    dy = goal_map.point.y - pose.linear.y
    ...
    yr = -np.sin(pose.angular.z)*dx + np.cos(pose.angular.z)*dy
...
def main():
    subpose = rospy.Subscriber('odom_twist', Twist, set_pose) #estimated rig pose 
